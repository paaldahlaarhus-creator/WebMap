...
c8_reasons <- data_raw |>                                                                                   
  mutate(       
    ...
    reason_uncertain   = is_selected(C8_newr8)
  ) |>
  dplyr::select(uuid, reason_protest, reason_truezero, reason_uncertain)

# 4. Classify respondents (step 1 - checkbox only)
# Gate: sq_all == TRUE (always-SQ)
# Priority order: Protest > True zero > Uncertain > Unclassified
response_classification <- sq_counts |> 
  ...
  mutate(
    response_type = case_when(
      ...
      sq_all & reason_uncertain    ~ "Uncertain",
      sq_all                       ~ "Unclassified",
      TRUE                         ~ "Non-sq_all"
    ),
...
sq_comments_review <- response_classification |> 
  ...
  dplyr::select(uuid, n_sq, response_type, reason_protest,
         reason_truezero, reason_uncertain) |>
